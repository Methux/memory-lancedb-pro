...
const memoryLanceDBProPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Log cross-group config warnings (non-fatal)
    const configWarnings: string[] = (config as any)._configWarnings || [];
    for (const w of configWarnings) {
      api.logger.warn(`memory-lancedb-pro: config: ${w}`);
    }
    ...
    // ========================================================================
    // LLM Fallback Judge for autoCapture
    // ========================================================================

    interface LLMJudgeResult {
      text: string;
      category: "fact" | "decision" | "preference" | "entity";
    }

    async function judgeByCaptureModel(
      texts: string[],
      ...
      try {
        ...
        if (!resp.ok) {
          api.logger.debug?.(`autoCaptureLLM: HTTP ${resp.status}`);
          return [];
        }

        // Ollama native API returns { message: { content: string } }
        ...
        // Parse JSON — handle both {"results":[...]} wrapper and bare [...]
        const parsed_json = JSON.parse(raw);
        const items: { i: number; keep: boolean; cat: string }[] =
          Array.isArray(parsed_json) ? parsed_json :
          Array.isArray(parsed_json?.results) ? parsed_json.results :
          [];

        const results: LLMJudgeResult[] = [];
        const validCats = new Set(["fact", "decision", "preference", "entity"]);
        ...
        for (const item of items) {
          if (
            ...
            const cat = validCats.has(item.cat)
              ? (item.cat as LLMJudgeResult["category"])
              : "fact";
            results.push({ text: texts[item.i - 1], category: cat });
          }
        }

        if (results.length > 0) {
          api.logger.info(
            `autoCaptureLLM: approved ${results.length}/${texts.length} texts`,
          );
        }

        return results;
      } catch (err: unknown) {
        // Fail-open: timeout, network error, parse error → skip silently
        const errMsg = err instanceof Error ? err.message : String(err);
        api.logger.debug?.(`autoCaptureLLM: skipped (${errMsg})`);
        return [];
      }
    }
    ...
    if (config.autoCapture !== false) {
      api.on("agent_end", async (event, ctx) => {
        ...
        try {
          ...
          // Only evaluate the last 6 messages to avoid re-processing old history
          ...
          let toCapture = [...regexMatched];
          const llmCategories = new Map<string, string>();

          if (regexMissed.length > 0 && config.autoCaptureLLM?.enabled) {
            const llmResults = await judgeByCaptureModel(regexMissed, config.autoCaptureLLM);
            for (const r of llmResults) {
              toCapture.push(r.text);
              llmCategories.set(r.text, r.category);
            }
          }
          ...
          // Clean up conversation-history formatting artifacts before storing
          const timestampPrefixPattern = /^\[(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s+\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}\s+GMT[+-]\d+\]\s*/;
          const cleanedCaptures = toCapture.map((text) => {
            const cleaned = text.replace(timestampPrefixPattern, "").trim();
            return cleaned || text; // fallback to original if stripping empties it
          });

          // Remap LLM categories from original→cleaned text keys
          const cleanedLlmCategories = new Map<string, string>();
          for (let i = 0; i < toCapture.length; i++) {
            const orig = toCapture[i];
            if (llmCategories.has(orig)) {
              cleanedLlmCategories.set(cleanedCaptures[i], llmCategories.get(orig)!);
            }
          }

          // Store each capturable piece (limit to 3 per conversation)
          const batch = cleanedCaptures.slice(0, 3);
          const categories = batch.map((text) =>
            cleanedLlmCategories.has(text)
              ? (cleanedLlmCategories.get(text) as "preference" | "fact" | "decision" | "entity")
              : detectCategory(text),
          );

          // Batch embed all texts in one API call (N→1 roundtrip)
          ...
          for (let i = 0; i < batch.length; i++) {
            ...
            if (mdMirror) {
              try {
                ...
                api.logger.warn(
                  `memory-lancedb-pro: md-mirror write failed (auto-capture), continuing: ${String(mirrorErr)}`,
                );
              }
            }
          }
        ...
        try {
          ...
          if (mdMirror) {
            try {
              ...
              api.logger.warn(
                `memory-lancedb-pro: md-mirror write failed (session-memory), continuing: ${String(mirrorErr)}`,
              );
            }
          }
